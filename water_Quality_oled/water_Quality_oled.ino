

#include "scrc_oled.h"
#include "Wire.h"

Adafruit_SH1106 display(I2C_SDA , I2C_SCL); // @suppress("Abstract class cannot be instantiated")

struct sensors_data display_buff[DISPLAY_BUF_COUNT];

short int idx_display_primary = 0;

const unsigned char iiith [] PROGMEM = {

    };


const unsigned char bitmap_scrc [] PROGMEM = {
    

};


const unsigned char bitmap_temp [] PROGMEM = {
  
  };

const unsigned char sample_water [] PROGMEM = {
 
};




void hw_setup_oled()
{
  pinMode(I2C_SCL, INPUT_PULLUP);           // set pin to input
  pinMode(I2C_SDA, INPUT_PULLUP);           // set pin to input
  display.begin(SH1106_SWITCHCAPVCC, OLED_ADDR);
  display.display();
  delay(2000);
  display.clearDisplay();
}

void display_oled(const struct sensors_data *display_buff, int display_idx)
{
  display.clearDisplay();
    display.display();
    display.drawBitmap(0, 0,  iiith, 128, 64, WHITE);
    display.display();
     delay(10000);

    // display SCRC in round rect
    /*display.clearDisplay();
    display.setTextColor(WHITE);
    display.setCursor(0,0);
    display.fillRoundRect(0, 2, 128, 64, 8, WHITE);
    display.setTextColor(BLACK);
    display.setTextSize(5);
    display.setCursor(6,10);
    display.println("SCRC");
    display.display();
    delay(10000);
*/
    // display scrc logo
     display.clearDisplay();
     display.drawBitmap(0, 0,  , 128, 64, WHITE);
     display.display();
     delay(10000);


    // display node name in round rect
    display.setCursor(0,0);
    display.fillRoundRect(0, 2, 128, 64, 8, WHITE);
    display.setTextColor(BLACK);
    display.setTextSize(3);
    display.setCursor(6,10);
    display.println("SR-wQ-SPCRC");
    display.display();
    delay(5000);

    // TDS
    display.clearDisplay();
    display.display();
    display.drawBitmap(0, 0,  sample_water, 64, 64, WHITE);
    display.display();
    display.setCursor(70,0);
    display.setTextSize(2.5);
    display.setTextColor(WHITE);
    display.println(" TDS : ");
    display.setCursor(70,30);
    display.setTextSize(2);
    display.setTextColor(WHITE);
    display.println(String(display_buff[display_idx].tdswithtemp));
    display.setCursor(70,50);
    display.setTextSize(2);
    display.setTextColor(WHITE);
    display.println("ppm");

    display.display();
    delay(5000);

    // Temperarture
    display.clearDisplay();
    display.display();
    display.drawBitmap(0, 0,  bitmap_temp, 64, 64, WHITE);
    display.display();

    display.setCursor(68,24);
    display.setTextSize(2);
     display.setTextColor(WHITE);
    display.println(String(display_buff[display_idx].temp));
    display.display();
    delay(5000);

    }
