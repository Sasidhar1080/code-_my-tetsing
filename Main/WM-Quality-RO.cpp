...
void loop_ro() {
	 ...
	 * 2. read from hw
	 * 3. publish to onem2m
	 * 4. publish to thingspeak
	 */

	debug_info("Am inside Loop");
	//debug_info("10(lRO) idx_data_primary  "+ String(idx_data_primary));

	wifi_connected = nw_is_connected();
	// read data from h/w
	read_data_from_hw_RO();

	/*
	 * n/w available and pub to onem2m is due, publish primary data
	 */
	if (wifi_connected && is_om2m_pub_due(millis())) {
		//debug_info("14 (pub) idx_data_primary  "+ String(idx_data_primary));
		...
		//debug_info("15(APB) pub_count  "+ String(pub_count));

		//debug_info("15(APB) idx_data_primary  "+ String(idx_data_primary));

		//update the last pub time
		last_om2m_pub = millis();
		debug_info("15 last_om2m_pub   "+ String(last_om2m_pub ));
	}
...
void read_data_from_hw_RO() {
	...
	if (millis() - last_ms_read_data_primary > TIME_INTERVAL_PRIMARY) {
		...
		if (idx_data_primary == PRIMARY_BUF_COUNT - 1) {
			...
			idx_data_primary++;
		debug_info("12 idx_data_primary  "+ String(idx_data_primary));
