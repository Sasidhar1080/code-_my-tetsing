...
int publish_onem2m(const struct sensors_data *ptr_sensors_data) {
...
#if defined(WM_WD_RO)
	// Data Format
	sprintf(data, "[%lld, %.2f, %.6lf, %.2f, %.2f, %.2lf]",
			ptr_sensors_data->time_stamp, ptr_sensors_data->temp,
			ptr_sensors_data->tdsvoltage, ptr_sensors_data->tdswithouttemp,
			ptr_sensors_data->tdswithtemp);
#endif

 #if defined(WM_WD_OH)
	sprintf(data, "[%lld, %.2lf, %.f, %.2f]", ptr_sensors_data->time_stamp,
			ptr_sensors_data->level, ptr_sensors_data->pH,
			ptr_sensors_data->turbidity);
#endif

#if defined(WM_WD_OH_ALL)
	sprintf(data, "[%lld, %.2f, %.6lf, %.2f, %.2f, %.2lf, %.2f, %.2f]",
			ptr_sensors_data->time_stamp, ptr_sensors_data->temp,
			ptr_sensors_data->tdsvoltage, ptr_sensors_data->tdswithouttemp,
			ptr_sensors_data->tdswithtemp, ptr_sensors_data->level,
			ptr_sensors_data->pH, ptr_sensors_data->turbidity);
#endif

#if defined(WM_WF)
	sprintf(data, "[%lld, %.2f, %.2lf, %.2f, %.6f]",
			ptr_sensors_data->time_stamp, ptr_sensors_data->flowrate,
			ptr_sensors_data->totalflow, ptr_sensors_data->pressure,
			ptr_sensors_data->pressurevoltage);
	...
	int node_id = ptr_sensors_data->node_id;
	...
	String data_cont;
	if (node_id == 0) {
			label =  String() + OM2M_DATA_LBL_0;
			data_cont = String() + OM2M_DATA_CONT_0;
		}
#if defined(WM_WF)
		else if (node_id == 1) {
			label = String() + OM2M_DATA_LBL_1;
			data_cont = String() + OM2M_DATA_CONT_1;
		} else if (node_id == 2) {
			label = String() + OM2M_DATA_LBL_2;
			data_cont = String() + OM2M_DATA_CONT_2;
		} else if (node_id == 3) {
			label = String() + OM2M_DATA_LBL_3;
			data_cont = String() + OM2M_DATA_CONT_3;
		}
	#endif
		else {
			//throw error
			debug_err("Invalid OM2M Node-ID");
			return -1;
		}
	...
	String link = String() + OM2M_MN + OM2M_AE + "/" + data_cont;
