...
//if(!wifi_connected)
	//retry_wifi();
	nw_start(STARTUP_WIFI_TIMEOUT, // @suppress("Invalid arguments")
...
void read_data_from_hw_OH() {
	...
	if (millis() - last_ms_read_data_primary > TIME_INTERVAL_PRIMARY) {
		...
		hw_read_level(
				&data_primary[idx_data_primary % PRIMARY_BUF_COUNT].level);

		hw_read_turbidity(
				&data_primary[idx_data_primary % PRIMARY_BUF_COUNT].turbidity);
